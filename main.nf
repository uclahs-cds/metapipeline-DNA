...
process create_input_csv_metapipeline_DNA {
    ...
    } else if (file_type.equals('FASTQ')) {
...
workflow {
    if (params.input?.BAM) {
        ich = Channel.from(params.input.BAM)
            .map { ['BAM', it.patient, it.sample, it.state, it.site, [it.patient, it.sample, it.state, it.site, it.path]] }
        ...
        ich = Channel.from(params.input.FASTQ)
            .map { ['FASTQ', it.patient, it.sample, it.state, it.site, [it.index, it.read_group_identifier, it.sequencing_center, it.library_identifier, it.platform_technology, it.platform_unit, it.bam_header_sm, it.lane, it.read1_fastq, it.read2_fastq]] }
            .groupTuple(by:[0,1,2,3,4])
        create_input_csv_metapipeline_DNA(ich)
