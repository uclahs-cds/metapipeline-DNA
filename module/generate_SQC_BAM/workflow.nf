...
workflow generate_SQC_BAM {
    ...
    main:
        if (!params.generate_SQC_BAM.is_pipeline_enabled) {
            ...
            identify_generate_sqc_bam_outputs(
                modification_signal.until{ it == 'done' }
                    .mix( run_generate_SQC_BAM.out.identify_generate_sqc_bam_out )
            )

            run_generate_SQC_BAM.out.complete
                .mix( identify_generate_sqc_bam_outputs.out.och_generate_sqc_bam_identified )
                .mix( pipeline_predecessor_complete )
                .collect()
                .map{ it ->
                    mark_pipeline_complete(params.this_pipeline);
