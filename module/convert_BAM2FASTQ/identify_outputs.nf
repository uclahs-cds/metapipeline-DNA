...
workflow identify_convert_bam2fastq_outputs {
    ...
    och_convert_BAM2FASTQ.map{ bam2fastq_out ->
        bam2fastq_out[3].each { read_group_csv, output_dir ->
            new File(read_group_csv.toRealPath().toString()).eachLine{ line, line_num ->
                ...
                params.sample_data[bam2fastq_out[1]]['convert-BAM2FASTQ'].add(
                    [
                        ...
                        'sample': rg_info[5],
                        'lane': rg_info[6],
                        'read1_fastq': identify_file("${output_dir}/${rg_info[0]}_collated_R1.fq.gz"),
                        'read2_fastq': identify_file("${output_dir}/${rg_info[0]}_collated_R2.fq.gz")
                    ]
                );
